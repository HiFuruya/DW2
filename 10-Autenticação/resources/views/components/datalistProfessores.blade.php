            ...
            @foreach ($data as $item)
                <tr>
                    ...
                    @foreach($fields as $field)
                        @if(trim($field) == trim('eixo_id'))
                            <td class="d-none d-md-table-cell">{{ $item->eixo->nome}}</td>
                        @elseif(trim($field) == trim('ativo'))
                            @if($item[$field] == 1)
                    ...
                    <td>
                        ...
                        @if(UserPermissions::isAuthorized('professores.show'))
                            <a nohref style="cursor:pointer" onclick="showInfoModal(['NOME: {{$item->nome}}', 'EMAIL: {{$item->email}}', 'SIAPE: {{$item->siape}}', 'EIXO/ÁREA: {{$item->eixo->nome}}'])" class="btn btn-primary">
