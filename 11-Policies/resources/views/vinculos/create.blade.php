@extends('templates.middleware', ['titulo' => "Docência (2022)"])
...
<div class="row ">
    <div class="col">
        <form action="{{ route('vinculos.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col">
                    <div class="form-floating mb-3 ">
                        <select name="disciplina_id" class="form-control {{ $errors->has('disciplina_id') ? 'is-invalid' : '' }}">
                            <option value=""></option>
                            @foreach($disciplinas as $i)
                                @php $cont = 0; @endphp

                            @foreach($vinculos as $vinculo)
                                @if($vinculo->disciplina_id == $i->id)
                                    @php $cont++; @endphp
                                @endif
                            @endforeach
            ...
            <div class="row">
                <div class="col">
                    <a href="{{route('vinculos.index')}}" class="btn btn-secondary btn-block align-content-center w">
